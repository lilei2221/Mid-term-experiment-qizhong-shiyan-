...
import android.content.ContentValues;  // 用于向数据库插入笔记数据
import android.content.SharedPreferences;  // 用于存储和检索 SharedPreferences 数据
import android.graphics.Color;  // 用于设置背景颜色
...
public class NotesList extends ListActivity {
    ...
    private static final String[] PROJECTION = new String[] {
            ...
            NotePad.Notes.COLUMN_NAME_CREATE_DATE  // 2 - 添加时间戳字段
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences prefs = getSharedPreferences("NotePadPrefs", MODE_PRIVATE);
        boolean hasInsertedSamples = prefs.getBoolean("hasInsertedSamples", false);

        if (!hasInsertedSamples) {
            insertSampleNotes();
            SharedPreferences.Editor editor = prefs.edit();
            editor.putBoolean("hasInsertedSamples", true);
            editor.apply();
        }

        // 初始化笔记列表
        initNotesList(null, null);// 初始时不带查询条件
        ...
        int[] viewIDs = { android.R.id.text1, R.id.note_timestamp };
    ...
    // 在方法签名中添加 `String sortColumn` 参数
    private void initNotesList(String query, String sortColumn) {
        ...
        // 使用 `sortColumn` 变量来设置排序顺序
        String orderBy = (sortColumn != null) ? sortColumn + " ASC" : NotePad.Notes.DEFAULT_SORT_ORDER;
        ...
        if (cursor != null) {
            ...
            Log.e(TAG, "Failed to retrieve data for notes list");
        }
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                // 提交查询时更新列表
                currentQuery = query;
                initNotesList(query, null); // 指定查询条件但不进行排序
            ...
            public boolean onQueryTextChange(String newText) {
                ...
                initNotesList(newText, null); // 指定查询条件但不指定排序列
        ...
        case R.id.menu_paste:
          ...
          return true;
            case R.id.menu_sort_by_color:
                initNotesList(currentQuery, NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR);
                return true;
            case R.id.menu_sort_by_creation_date:
                initNotesList(currentQuery, NotePad.Notes.COLUMN_NAME_CREATE_DATE);
                return true;
        default:
