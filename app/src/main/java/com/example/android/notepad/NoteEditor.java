...
import android.app.AlertDialog; // 用于显示背景颜色选择对话框
import android.content.ContentValues; // 用于更新数据库中的背景颜色字段
import android.database.Cursor; // 用于查询数据库
import android.graphics.Color; // 用于设置背景颜色（如Color.WHITE, Color.YELLOW等）
import android.net.Uri; // 用于处理内容 URI
...
public class NoteEditor extends Activity {
    ...
    private int selectedBackgroundColor = Color.WHITE; // 默认背景颜色
    /*
    ...
    private static final String[] PROJECTION =
        new String[] {
            ...
            NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR // 3 - 笔记背景颜色（如果需要保存背景颜色）
        };
    ...
    private static final int COLUMN_INDEX_NOTE = 2; // 新增笔记内容的索引
    private static final int COLUMN_INDEX_BACKGROUND_COLOR = 3; // 新增背景颜色的索引（可选）
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            String noteText = mCursor.getString(COLUMN_INDEX_NOTE);
            int backgroundColor = mCursor.getInt(mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR));
    ...
    private void showBackgroundColorDialog() {
        final int[] colors = {Color.WHITE, Color.YELLOW, Color.LTGRAY, Color.GREEN}; // 示例颜色
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Select Background Color");
        builder.setItems(new CharSequence[]{"White", "Yellow", "Gray", "Green"}, (dialog, which) -> {
            selectedBackgroundColor = colors[which]; // 保存用户选择的颜色
            mText.setBackgroundColor(selectedBackgroundColor); // 立即更新背景
        });
        builder.show();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case R.id.menu_revert:
            cancelNote();
            break;
            case R.id.menu_change_background: // 新增的背景更换选项
                showBackgroundColorDialog(); // 调用显示背景颜色选择对话框的方法
                break;
        }
...
//END_INCLUDE(paste)
    ...
    private final void updateNote(String text, String title) {
        ...
        values.put(NotePad.Notes.COLUMN_NAME_BACKGROUND_COLOR, selectedBackgroundColor); // 保存背景色
